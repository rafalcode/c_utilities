/* bgfiltf: this takes a mac signalt bed file, whose fourth column has a float value and filters out those under a certain signal value */
#include<stdio.h>
#include<stdlib.h>
#include<string.h>
...
int main(int argc, char *argv[])
{
	...
	if(argc!=3) {
		...
		printf("1) belong to same contig 2) are contiguous 3) satisfy a min signal value. Output is bedgraph to STDOUT.\n");
		printf("Usage. Pls supply 2 arguments 1) name of bedgraph file 2) minimum singal value (float).\n");
		exit(EXIT_FAILURE);
	}
	float minsig=atof(argv[2]);
	int i, j, m, n;
	...
	/*
	int dcasz;
	i4_t *dca=difca(bgrow, m, &dcasz, minsig);

	bgr_t **bgra=malloc(dcasz*sizeof(bgr_t*));

	bgra[0]=malloc(dca[0].sc*sizeof(bgr_t));
