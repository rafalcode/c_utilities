...
void prtaawcsel0(aaw_c *aawc) /* selective print 0, debugging version */
{
    int i;
    for(i=0;i<aawc->numl;++i) {
        printf("%s\t", aawc->aaw[i]->aw[1]->w + aawc->aaw[i]->chrdp);
        printf("%i\t", aawc->aaw[i]->pos);
        printf("%i\t%i\t", aawc->aaw[i]->sqp[0], aawc->aaw[i]->sqp[1]);
        printf("%s\n", aawc->aaw[i]->aw[5]->w);
    }
}

void prtaawcsel2(aaw_c *aawc) /* selective print 2 with ref allele */
{
    /* still a debugging function shows how the right allele between the square brackerts is chosen */
    int i;
    unsigned char aoff; // allele offset, will be 2 to for first, 4 for second (ref I think)
    unsigned char hdoff; // header offset, if there is a header it will be 1
    aoff=3;
    hdoff=1;
    for(i=hdoff;i<aawc->numl;++i) {
        printf("%s\t", aawc->aaw[i]->aw[1]->w + aawc->aaw[i]->chrdp);
        printf("%i\t", aawc->aaw[i]->pos);
        printf("%.*s\t", aawc->aaw[i]->sqp[0], aawc->aaw[i]->aw[5]->w); // print up to opening sqbrack
        putchar(' ');
        putchar(aawc->aaw[i]->aw[5]->w[aawc->aaw[i]->sqp[0] + aoff]);
        putchar(' ');
        printf("%s\t", aawc->aaw[i]->aw[5]->w + 1 + aawc->aaw[i]->sqp[1]);
    }
}

void prtaawcsel3(aaw_c *aawc) /* selective print 2 with ref allele */
{
    /* still a debugging function shows how the right allele between the square brackerts is chosen */
    int i;
    unsigned char aoff; // allele offset, will be 2 to for first, 4 for second (ref I think)
    ...
    for(i=hdoff;i<aawc->numl;++i) {
        printf("%s\t", aawc->aaw[i]->aw[1]->w + aawc->aaw[i]->chrdp);
        printf("%i\t", aawc->aaw[i]->pos);
        printf("[@%i\t", aawc->aaw[i]->sqp[0]);
        printf("]+1@%i\t", aawc->aaw[i]->sqp[1]+1);
        printf("rest:%i\t", aawc->aaw[i]->aw[5]->lp1 - aawc->aaw[i]->sqp[1]-1);
        putchar(aawc->aaw[i]->aw[5]->w[aawc->aaw[i]->sqp[0] + aoff]);
        putchar('\n');
    }
}

void prtaawcfar(aaw_c *aawc) /* print as fasta with ref allele */
{
    /* still a debugging function shows how the right allele between the square brackerts is chosen */
    int i;
    unsigned char aoff; // allele offset, will be 2 to for first, 4 for second (ref I think)
    ...
    hdoff=1;
    for(i=hdoff;i<aawc->numl;++i) {
        printf("> %s_", aawc->aaw[i]->aw[0]->w); // name of SNPFlaseq (flanking sequence)
        printf("C%s_", aawc->aaw[i]->aw[1]->w + aawc->aaw[i]->chrdp);
        printf("P%i_", aawc->aaw[i]->pos);
        printf("%c_", aawc->aaw[i]->aw[3]->w[0]); // allele 1
        putchar(aawc->aaw[i]->aw[4]->w[0]); // allele 2
        putchar('\n');
        printf("%.*s", aawc->aaw[i]->sqp[0], aawc->aaw[i]->aw[5]->w); // print up to opening sqbrack
        putchar(aawc->aaw[i]->aw[5]->w[aawc->aaw[i]->sqp[0] + aoff]); // THE allele
        printf("%s", aawc->aaw[i]->aw[5]->w + 1 + aawc->aaw[i]->sqp[1]);
        putchar('\n');
    }
}

void prtaawcfaa(aaw_c *aawc) /* print as fasta with alt allele */
{
    /* still a debugging function shows how the right allele between the square brackerts is chosen */
    int i;
    unsigned char aoff; // allele offset, will be 2 to for first, 4 for second (ref I think)
    unsigned char hdoff; // header offset, if there is a header it will be 1
    aoff=1; // yes, the lower number is the alt allele.
    hdoff=1;
    for(i=hdoff;i<aawc->numl;++i) {
        printf("> %s_", aawc->aaw[i]->aw[0]->w); // name of SNPFlaseq (flanking sequence)
        printf("C%s_", aawc->aaw[i]->aw[1]->w + aawc->aaw[i]->chrdp);
    ...
    while( (c=fgetc(fp)) != EOF) {
        if( (c== '\n') | (c == ' ') | (c == '\t') ) {
            ...
            if( inword==1) { /* cue word-ending procedure */
                ...
                if(couw == 2) {
                    aawc->aaw[aawc->numl]->pos = atoi(aawc->aaw[aawc->numl]->aw[couw]->w);
                }
            ...
            aawc->aaw[aawc->numl]->aw[couw]->w[couc++]=c;
            if((couw == 1) & (c=='r')) {
                aawc->aaw[aawc->numl]->chrdp = couc;
            } else if((couw == 5) & (c=='[')) {
                aawc->aaw[aawc->numl]->sqp[0] = couc-1;
            } else if((couw == 5) & (c==']')) {
                aawc->aaw[aawc->numl]->sqp[1] = couc-1;
            }
...
int main(int argc, char *argv[])
{
    ...
    printf("DBG in force\n"); 
    printf("This was for checking size of flanking seq, it's 200 on ether side, making 401 in total\n");
#else
    // prtaawcdata(aawc); // just the metadata
    prtaawcfar(aawc); // printout original text as well as you can.
#endif
    // printf("Numlines: %zu\n", aawc->numl); 
