...
EXES=cmd2ext extcou cleangrpo matread genread genread_d txtread txtread_d parard ahread ahread_d dreadn dreadn_d vcolfrcr volfrcr_d txtread_t bgread bgread2 bgread0 bgread_ bgread0a bgread3 bgmergmc bgmergmc_d bgmergmcstealth bgreadx bgreadx0 bgfiltf contabrd rg0 macsigf bedsumzr bgmergbl bgmergbl2 vttmrg pwmatr tma2pwma pedread dcou dcou2 dcou3 mapedstats mapedstats_d pedcmp pedcmp_d mprd3 mprd3_d mpdmu mpdmu_d dcou4 pedsta pedsta0 mpdmu2 mpdmu2_d mpdmu3 mpdmu3_d mpdmu4 mpdmu4_d tpedsta_d bglsta bglsta_d bglvset vttrd vttgo3 vttgo2 tablerd genrd ssrd ssrd_d blard blard_d blard2 blard2_d rurd fard fard_d fard2 paredown f3 f4 pare0 pare2 pare3 onel morel vrd0 chktasty tabcmatch tpedsta2 namgenes2 namgenes namsets repinfam csvrd fcsvrd0 fcsvrd fintimrd csvrdh srtfix sentin sentinu u0 tmsee pread pread0 rdog0 csvrde csvrdm csvrdmcl
...
# line index based subititle printer
# this is strictly for yt-dlp downloaded video subtitles
# very delicate, depends on line indices, but I think they are fairly consistet
# vttgo2: just takes one file
# vttgo3: takes two subtitles and interleaves them
vttgo2: vttgo2.c
	${CC} ${CFLAGS} -o $@ $^
vttgo3: vttgo3.c
	${CC} ${CFLAGS} -o $@ $^
...
# initially table red is to take in a <numbertiming> <ifilename> <number timing> table
# # but it wouldn't take much to adapt it to other tables.
tablerd: tablerd.c
	${CC} ${CFLAGS} -o $@ $^
...
namgenes: namgenes.c
	${CC} ${CFLAGS} -o $@ $^
# the problem with gene name conversion is that there are always names missing or even duplicated.
# this second version takes two tsv files i..e two conversion tables.
# they're both a bit particular so be careful
namgenes2: namgenes2.c
	${CC} ${CFLAGS} -o $@ $^
pare3: pare3.c
	${CC} ${CFLAGS} -o $@ $^
# and the winner is...
...
pread0: pread0.c
	${CC} ${CFLAGS} -o $@ $^
# above prog really isn't focussed enough. Quit looking for numbers! This is about paragraphs.
pread: pread.c
	${CC} ${CFLAGS} -o $@ $^
