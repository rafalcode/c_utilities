...
void reall_wc(w_c **wc, unsigned *cbuf)
{
    ...
    tcbuf += CBUF;
    twc->lp1=tcbuf;
    twc->w=realloc(twc->w, tcbuf*sizeof(int));
        ...
        if( (c== '\n') | (c == ' ') | (c == '\t') | (c == '.')) {
            if( inword==1) { /* cue word-ending procedure */
                if( (c == ' ') | (c == '\t'))
                    ...
                    aawc->aaw[aawc->numl]->aw[couw]->w[couc++]=' ';
            ...
            if(c=='.') { /* cue line-ending procedure */
                if(aawc->numl ==lbuf-1) {
                    lbuf += LBUF;
...
int main(int argc, char *argv[])
{
    /* argument accounting */
    if(argc!=2) {
        printf("%s, a program to read a test file as sentences, that is ending with .\n", argv[0]);
