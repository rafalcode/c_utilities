...
void prtaawcfw(aaw_c *aawc) /* print final word */
{
    int i;
    for(i=0;i<aawc->numl;++i) {
	    // printf("Finalword=\"%s\"\n", aawc->aaw[i]->aw[aawc->aaw[i]->al-1]->w);
	    printf("minsecs=%i:%i\n", aawc->aaw[i]->mins, aawc->aaw[i]->secs);
    }
}

void prtaawcfwcu(aaw_c *aawc) /* print final word */
{
    int i;
    int cumin=0;
    int cusec=0;
    for(i=0;i<aawc->numl;++i) {
	    // printf("Finalword=\"%s\"\n", aawc->aaw[i]->aw[aawc->aaw[i]->al-1]->w);
	    printf("minsecs=%i:%i ", aawc->aaw[i]->mins, aawc->aaw[i]->secs);
	    cumin += aawc->aaw[i]->mins;
	    cusec += aawc->aaw[i]->secs;
        if(cusec>=60) {
	        cumin++;
	        cusec -= 60;
        }
...
void lwdur(aaw_c *aawc) /* last word is duration */
{
    int i, j;
	char t=':';
	char *tk=NULL;
    for(i=0;i<aawc->numl;++i) {
	    tk=strtok(aawc->aaw[i]->aw[aawc->aaw[i]->al-1]->w, &t);
	    aawc->aaw[i]->mins=atoi(tk);
	    aawc->aaw[i]->secs=atoi(tk+2);
    }
}
...
int main(int argc, char *argv[])
{
    ...
    lwdur(aawc); // printout original text as well as you can.
    prtaawcfwcu(aawc); // printout original text as well as you can.
    printf("Numlines: %zu\n", aawc->numl); 
