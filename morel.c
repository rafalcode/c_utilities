...
void prtaawcplain(aaw_c *aawc) /* print line and word details, but not the words themselves */
{
    ...
    for(i=0;i<aawc->numl;++i) {
        ...
        for(j=0;j<aawc->aaw[i]->al;++j) {
            printf("w%il%i:", j, aawc->aaw[i]->aw[j]->lp1-1);
...
aaw_c *processinpf(char *fname)
{
    ...
    int i;
    int numnl=0, oldnumnl=-1;
    size_t couc /*count chars per line */, couw=0 /* count words */;
    int c, oldc='\0';
    boole inword=0;
    boole intitle=0;
    boole ona1=1; // on arrow 1 by default must start as 1
    ...
    while( (c=fgetc(fp)) != EOF) {
        ...
        if( intitle & ((c ==' ') | (c == '\t') | (c=='=') | (c=='|')) ) {
            if( inword==1) { /* cue word-ending procedure */
                ...
                inword=0;
                if(numnl == oldnumnl+1)// therefore, it's the last letter in the last word of an intitle
                    intitle=0;
            }
