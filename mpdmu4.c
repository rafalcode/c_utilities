#include "mpdmu4.h" // has the header stuff, and structs.
...
w_c *crea_wc(void)
{
    ...
    wc->wbf=CBUF;
    wc->t=STRG;
    wc->w=calloc(wc->wbf, sizeof(char));
    return wc;
}

void grow_wc(w_c **wc, unsigned buf)
{
    w_c *twc=*wc;
    twc->wbf += buf;
    twc->w=realloc(twc->w, twc->wbf*sizeof(char));
    memset(twc->w+twc->wbf-buf, 0, buf*sizeof(char));
    *wc=twc; /* realloc can often change the ptr */
    return;
}

void condgrow_wc(w_c **wc, unsigned idx)
{
    unsigned diffsz; // on occasions if normalized
    w_c *twc=*wc;
    if(idx == twc->wbf-1) {
        twc->wbf += CBUF;
        diffsz = twc->wbf - twc->lp1;
        twc->w=realloc(twc->w, twc->wbf*sizeof(char));
        memset(twc->w+twc->lp1, 0, diffsz*sizeof(char));
        *wc=twc; /* realloc can often change the ptr */
    }
    return;
}
...
void norm_wc(w_c **wc)
{
    ...
    if(twc->lp1 > twc->wbf) {
        diffsz = twc->lp1 - twc->wbf;
        memset(twc->w+twc->wbf, 0, diffsz*sizeof(char));
    }
    twc->wbf = twc->lp1;
    // printf("normalising %s for %u\n", twc->w, twc->lp1); 
    *wc=twc; /* realloc can often change the ptr */
    return;
}
...
snodw **tochainharr1(wff_t *wff, int m, unsigned tsz)
{
    // this "2" version of the function hashes on nn, the C##_P###etc names
    unsigned i;

    snodw **stab=malloc(tsz*sizeof(snodw *));
    for(i=0;i<tsz;++i) 
...
bo:
    for(i=0; i< m; ++i) {
        ...
        tsnod2=stab[tint];
        while( (tsnod2 != NULL) ) {
            if(!strcmp(tsnod2->wff->w, wff[i].w))
                goto bo;
            tsnod0=tsnod2;
            tsnod2=tsnod2->n;
        }
        tsnod0->n=malloc(sizeof(snodw));
        tsnod0->n->wff=wff+i;
        tsnod0->n->idx=i;
        tsnod0->n->n=NULL;
    }
...
void filtchainharr1(snod **stab, unsigned tsz, wff_t *wff, int m)
{
    unsigned i;
    boole inmap;
...
void prtchaharr1(snodw **stab, unsigned tsz)
{
    unsigned i;
    snodw *tsnod2;
    for(i=0;i<tsz;++i) {
...
void freechainharr1(snodw **stab, size_t tsz)
{
    int i;
    snodw *tsnod0, *tsnod2;
    for(i=0; i<tsz; ++i) {
...
aaw_c *process_mpaaw3(char *fname, snodw **stab, unsigned htsz, size_t *recfout)
{
    /* declarations */
    FILE *fp=fopen(fname,"r");
    int i;
...
int main(int argc, char *argv[])
{
    ...
    size_t recfout=0; // records filteredout 
    aaw_c *mpaaw=process_mpaaw3(argv[1], wha, htsz, &recfout);
    prtaawcplain(mpaaw);
    printf("Your snpname file had %i entries, and so filtered out %zu recs to produce %zu-record map file to %zu.\n", mnf, recfout, mpaaw->numl);

    /* OK, now we have the filter mp, we can get rid of the hash for the snp names,
     * that can be all forgotten now */
    freechainharr1(wha, htsz);
    for(i=0;i<mnf;++i)
        free(wff[i].w);
    free(wff);

    /* we going to do the dup analysis on ht e ... unf*/
    adia_t *ad=crea_adia();
