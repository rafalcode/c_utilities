...
void dupstats3(aaw_c *aawc, mp_t *mp, adia_t *ad, i2g_t2 *mid) // grab the best TRs
{
    char a1, a2;
    int j, jj, k;
    unsigned tu;
    ...
    for(i=0;i<aawc->numl;++i) {
        ...
        for(j=0;j<ad->sz;++j) { // for each dup category, group better said.
            ...
            prt_adgia(adg);
            // proc_adgia(adg, i2);
            proc_adgia3(adg, i2, mp, aawc, i, mid);
            free_adgia(adg);
        }
        norm_i2(i2);
        prt_i2(i2);
...
void prt_mp2(mp_t *mp, int m, int n, i2g_t2 *mid, aaw_c *aawc)
{
    int i, j, k=0, ii;
    for(i=0;i<m-10;++i) {
        if(mp[i].gd == 1) {
        ...
        if(mp[i].gd == 2) {
            printf("%c\t", gtna[(*mid->gt)[k]][0]);
            printf("%c\t", gtna[(*mid->gt)[k]][1]);
            k++;
        } else {
            ii=i*2+6;
            printf("%c\t", aawc->aaw[0]->aw[ii]->w[0]);
            printf("%c\t", aawc->aaw[0]->aw[ii+1]->w[0]);
        }
        printf("\n");
    }
    return;
...
int main(int argc, char *argv[])
{
    ...
    for(;;) {
        ...
        printf("sz aawc is %zu\n", aawc->numl);
        dupstats4(aawc, mp, ad, mid, &numsamps);
        prt_mp2(mp, m, n, mid, aawc);
        printf("\n"); 
        free_aawc(&aawc);
    }
    fclose(fp);
