...
void cmpaawc(aaw_c *aawc, aaw_c *aawc2)
{
    ...
    for(i=0;i<aawc->numl;++i) {
        ...
        for(ii=0;ii<aawc2->numl;++ii) {
            ...
            printf("\"%s\" present in both. Allele mismatches (if any): ", aawc->aaw[i]->aw[1]->w);
            for(j=6;j<aawc->aaw[i]->al;j+=2) {
                if(aawc->aaw[i]->aw[j]->w[0] != aawc2->aaw[ii]->aw[j]->w[0])
                    printf("c%i_%c_%c ", j, aawc->aaw[i]->aw[j]->w[0], aawc2->aaw[ii]->aw[j]->w[0]);
                if(aawc->aaw[i]->aw[j+1]->w[0] != aawc2->aaw[ii]->aw[j+1]->w[0])
                    printf("c%i_%c_%c ", j, aawc->aaw[i]->aw[j+1]->w[0], aawc2->aaw[ii]->aw[j+1]->w[0]);
            }
        }
        printf("\n");
    }
