/* tmsee.c "timings see" start off a YT  timing list (such as they have) */
#include<stdio.h>
#include<stdlib.h>
#include<string.h>
#include "tmsee.h"
...
void prti(char *fn, char *tmng)
{
    ...
    // printf("hs=%i ms=%i ss=%i fs=%i\n", hs, ms, ss, fs); 
    float t0=hs*3600+ms*60+ss+fs/1000.;
    s2="ffmpeg -i Tchaikovsky_-_Complete_Piano_Works_The_Seasons__For_the_young_+_P°__Cen._rec.__-_Viktoria_Postnikova_-hbpxoGi6NXA.opus -ss 3:03:27.0 -to 3:32:15.0 -map 0:a -c copy tchaiop39.opus";
    printf("%4.4f to %4.4f\n", t0, t0+dt); 
}
...
{
    int i, j;
    printf("Here we print the timing strings from the input text file, and on which columns they appear and their length.\n"); 
    for(i=0;i<aawc->numl;++i) {
        printf("L_%u_#W_%u: ", i, aawc->aaw[i]->al); 
        for(j=0;j<aawc->aaw[i]->al;++j) {
            // printf("l%ut", aawc->aaw[i]->aw[j]->lp1-1);
            if(aawc->aaw[i]->aw[j]->t == TMNG)
                // printf("%s len: %i\n", aawc->aaw[i]->aw[j]->w, aawc->aaw[i]->aw[j]->lp1);
                printf("%s(COL_%i_SZ_%i) ", aawc->aaw[i]->aw[j]->w, j, aawc->aaw[i]->aw[j]->lp1);
        }
        printf("\n"); 
    }
...
int main(int argc, char *argv[])
{
    /* argument accounting */
    if(argc!=2) {
        printf("%s, a program to read in YT timings style: timing and then tokens or text.\n", argv[0]);
        printf("Pls supply 1 arguments 1) name of text file.\n");
