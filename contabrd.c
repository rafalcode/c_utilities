/* Contingency Table Read ... and, obviously some calculations */
#include<stdio.h>
#include<stdlib.h>
#include<string.h>
#include<math.h>
...
typedef struct /* struct for the totals */
{
	float *r;
	float *c;
} tots_t;
...
void prtmatw(float *mat, int m, int n, tots_t tots) /* prijnt mat with */
{
	int i, j;
	...
	for(i=0;i<m;++i) {
		...
		printf("%4.4f ", tots.r[i]);
		printf("\n"); 
	}
	for(j=0;j<n;++j) 
		printf("%4.4f ", tots.c[j]);
	printf("\n");
	return;
}

void prtmat(float *mat, int m, int n)
{
	int i, j;
	printf("Matrix is %i rows by %i columns and is as follows:\n", m, n); 
	for(i=0;i<m;++i) {
...
float *ematcalc(int m, int n, tots_t tots) /* notw how this does not require the original matrix */
{
	int i, j;
	float *emat=malloc(m*n*sizeof(float));
	float gtot=0; /* grand total, summing the rows or columsn will do */
	for(i=0;i<m;++i)
		gtot += tots.r[i];

	for(i=0;i<m;++i)
		for(j=0;j<n;++j) 
			emat[i*n+j] = tots.c[j]*tots.r[i]/gtot;
	return emat;
}

float *chimatcalc(float *mat, float *emat, int m, int n) /* notw how this does not require the original matrix */
{
	int i, j;
	float *chimat=malloc(m*n*sizeof(float));

	for(i=0;i<m;++i)
		for(j=0;j<n;++j) 
			chimat[n*i+j] = pow(mat[n*i+j] - emat[i*n+j], 2) / emat[i*n+j];
	return chimat;
}
...
int main(int argc, char *argv[])
{
	...
	/*Ok now get the row totals */
	tots_t tots;
	tots.r=calloc(m, sizeof(float));
	tots.c=calloc(n, sizeof(float));
	...
	prtmatw(mat,m,n,tots);
	float *emat=ematcalc(m, n, tots);
	prtmat(emat,m,n);
	float *chimat=chimatcalc(mat, emat, m, n);
	prtmat(chimat,m,n);
