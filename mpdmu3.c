...
void reset_wc(w_c **wc)
{
    w_c *twc=*wc;
    twc->lp1 = 0;
    twc->wbf=CBUF;
    twc->w=realloc(twc->w, twc->wbf*sizeof(char));
    memset(twc->w, 0, twc->wbf*sizeof(char));
    *wc=twc; /* realloc can often change the ptr */
    return;
}

void norm_wc2(w_c **wc, unsigned prevsz)
{
    w_c *twc=*wc;
    twc->w=realloc(twc->w, twc->lp1*sizeof(char));
    if(twc->lp1 > prevsz)
        memset(twc->w+prevsz, 0, sizeof(char));
    printf("normalising %s for %u\n", twc->w, twc->lp1); 
    *wc=twc; /* realloc can often change the ptr */
    return;
}
            ...
            if( inword==1) { /* cue word-ending procedure */
                ...
                if(couw==1) {
                    ...
                    if( (stab[tint] == NULL) ) {
                        ...
                        couw=0;
                        inword=0;
                        reset_wc(aawc->aaw[aawc->numl]->aw+couw);
                        // reset_wc(aawc->aaw[aawc->numl]->aw+(couw-1));
                        continue;
                    }
                    tsnod2=stab[tint];
                    while( (tsnod2 != NULL) ) {
