/* bgmergmc: this takes a bed file, and a minimum coverage and merges all the segments that 1) belong to same contig 2) are contiguous 3) satisfy mincov
...
i4_t *difca(bgr_t *bgrow, int m, int *dcasz, int minco) /* An temmpt to merge bgraph quickly, no hope */
{
	...
	for(i=goodi+1;i<m;++i) {
		...
		if( (!strcmp(tstr, bgrow[i].n)) & (bgrow[i].c[0] == bgrow[dca[dci].lgbi].c[1]) & (bgrow[i].co >= minco) ) {
			dca[dci].sc++;
			dca[dci].lgbi=i;
